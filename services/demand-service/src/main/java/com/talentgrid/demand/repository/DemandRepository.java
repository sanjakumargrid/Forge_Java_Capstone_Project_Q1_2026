...
public interface DemandRepository extends JpaRepository<Demand, Long> {
    ...
    /**
     * Sum of required positions for demands created or filled in the given date range.
     * Date filtering: WHERE (created_at >= startDate OR demand has filled positions in range)
     */
    ...
    /**
     * Sum of required positions for demands created or filled in the given date range (with end date).
     * Date filtering: WHERE (created_at between startDate and endDate OR demand has filled positions in range)
     */
    @Query(value = "SELECT COALESCE(SUM(d.required_count), 0) FROM demands d " +
            "WHERE d.is_deleted = false " +
            "AND (CAST(d.created_at AS DATE) >= CAST(:startDate AS DATE) " +
    ...
    /**
     * Sum of filled positions (internal + external) for demands in the given date range.
     */
    ...
    /**
     * Average time-to-fill in days for demands that have filled positions in the given date range.
     * Time-to-fill = (earliest FILLED_* status transition timestamp - demand.created_at)
     * Uses demand_status_history to find the exact timestamp of the first FILLED_* transition.
     */
    ...
    /**
     * Minimum days to fill for demands that have filled positions in the given date range.
     */
    ...
    /**
     * Maximum days to fill for demands that have filled positions in the given date range.
     */
