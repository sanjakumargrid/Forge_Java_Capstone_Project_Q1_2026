...
public class WorkforceKafkaProducer {
    ...
    public void publishEmployeeResumeParsed(ResumeParsedPayload payload, String requestId) {
        BaseEvent<ResumeParsedPayload> event = new BaseEvent<>();
        String key = UUID.randomUUID().toString();
        event.setEventType("EMPLOYEE_RESUME_PARSED");
        event.setSource("workforce-service");
